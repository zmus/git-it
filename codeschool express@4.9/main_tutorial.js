...
var logger = require('./logger_middleware');  // './' -> LOCAL module (not an NPM module)

// ...mount it and add it to the stack
app.use(logger);
...
app.post('/people', parseUrlencoded, function (req, res) {
  ...
  people[req.body.name] = req.body.description;
...
/*------------------------------------------------------------------------------ 
  Static Routes with Query
    ...
    request.query.limit 
      = access 'limit' query string

    Object.keys() 
      = returns properties from the object
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------ 
  app.param() 
    - maps placeholders to callback functions
    - used for running PRE-CONDITIONS on Dynamic Routes
------------------------------------------------------------------------------*/

// called for all routes which include the ':name' placeholder
